"""Worker module for processing jobs in AIMQ.

This module provides the Worker class that handles job processing, including job
execution, error handling, and status updates.
"""
...
class WorkerThread(threading.Thread):
    ...
    def __init__(
        ...
        """Initialize the worker thread with the given parameters."""
        super().__init__()
        self.queues = queues
        self.logger = logger
            ...
            try:
                ...
                for queue in self.queues.values():
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Runtime error in queue {queue.name}", {"error": str(e)}
                        )
...
class Worker(BaseModel):
    ...
    name: str = Field(
        default_factory=lambda: config.worker_name, description="Name of this worker"
    )
    ...
    def assign(
        ...
        *,
        ...
        Args:
            ...
            timeout: Maximum time in seconds for a task to complete. If 0, messages will
                be popped instead of read
            delete_on_finish: Whether to delete (True) or archive (False) jobs after
                processing
    ...
    def task(
        ...
        *,
        ...
        """Register a function that returns a Runnable with a queue.
    ...
    def load(cls, worker_path: Path) -> "Worker":
        ...
        if not hasattr(module, "worker"):
            raise AttributeError(
                f"Module {worker_path} does not export a 'worker' attribute"
            )
