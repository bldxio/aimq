...
class ImageOCRInput(BaseModel):
    ...
    save_debug_image: bool = Field(
        default=False,
        description=(
            "If True, includes debug image in output showing detected text regions"
        ),
    )
...
class ImageOCR(BaseTool):
    ...
    def _run(self, image: Attachment, save_debug_image: bool = False) -> Dict[str, Any]:
        ...
        Returns:
            dict: OCR results including processing time, detected text, and debug image
                if requested
        """
        ...
        results = self.processor.process_image(
            image=image.data, save_debug_image=save_debug_image
        )

        return results

    async def _arun(
        self, image: Attachment, save_debug_image: bool = False
    ) -> Dict[str, Any]:
