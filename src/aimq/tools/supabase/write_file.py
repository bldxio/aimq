...
class WriteFileInput(BaseModel):
    ...
    path: Optional[str] = Field(
        None, description="The path values to apply to the template path"
    )
    bucket: Optional[str] = Field(
        "files", description="The storage bucket to read the file from"
    )
...
class WriteFile(BaseTool):
    ...
    formatter: Literal["f-string", "mustache"] = Field(
    ...
    def _get_template(self, value: str | PromptTemplate) -> PromptTemplate:
        ...
        if isinstance(value, PromptTemplate):
            return value
        return PromptTemplate.from_template(value, template_format=self.formatter)
    ...
    def _run(
        ...
        try:
            ...
            supabase.client.storage.from_(bucket).upload(
                path=path,
                file=file.data,
                file_options={"upsert": "true", "content-type": mimetype},
            )
