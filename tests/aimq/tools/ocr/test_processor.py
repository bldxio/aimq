...
from aimq.tools.ocr.processor import (
    OCRProcessor,
    boxes_overlap,
    group_text_boxes,
    merge_boxes,
...
class TestOCRProcessor:
    ...
    Tests initialization, reader management, image processing functionality,
    and error handling of the OCRProcessor class.
    """
    ...
    def test_process_image_invalid_input(self, ocr_processor):
        ...
        with pytest.raises(
            ValueError, match="Image must be a file path, PIL Image, or bytes"
        ):
            ocr_processor.process_image(None)


class TestBoxOperations:
    """Test suite for box operation utility functions.

    Tests the functionality of box manipulation utilities including overlap detection,
    ...
    def test_group_text_boxes(self):
        ...
        groups_with_growth = group_text_boxes(
            detections, width_growth=20, height_growth=20
        )
