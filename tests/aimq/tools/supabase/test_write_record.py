...
class TestWriteRecord:
    def test_init(self, write_record_tool):
        ...
        assert write_record_tool.description == "Write a record to Supabase. If an ID is provided, updates existing record; otherwise creates new record."
    ...
    def test_run_update_record(self, write_record_tool, mock_supabase):
        ...
        mock_table = Mock()
        mock_table.update.return_value.eq.return_value.execute.return_value = Mock(data=[expected_data])
