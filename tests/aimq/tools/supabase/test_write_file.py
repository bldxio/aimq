...
"""Tests for the Supabase file write tool.

This module contains test cases for the WriteFile tool, which is responsible for
writing files to Supabase Storage. Tests cover initialization, file writing
...
@pytest.fixture
def write_file_tool():
    """Create a WriteFile tool instance for testing."""
    return WriteFile()
...
class TestWriteFile:
    """Test suite for the WriteFile tool.

    Tests the functionality of the WriteFile tool including initialization,
    file writing with default and custom parameters, and error handling.
    """
    ...
    def test_upload_failure(self, write_file_tool, mock_supabase):
        ...
        with pytest.raises(
            ValueError, match="Error writing file to Supabase: Upload failed"
        ):
            write_file_tool._run(
                path=file_path, bucket="files", file=Attachment(data=file_content)
            )
