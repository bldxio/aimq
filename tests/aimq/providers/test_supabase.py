# type: ignore
"""Tests for the Supabase Queue Provider.

This module contains tests for the Supabase implementation of the queue provider,
including message handling, queue operations, and error conditions.
"""
...
def mock_supabase():
    ...
    Returns:
        Mock: A mocked Supabase client with pre-configured return values.
    """
...
@pytest.fixture
def provider():
    """Create a SupabaseQueueProvider instance for testing.

    Returns:
        SupabaseQueueProvider: A fresh instance of the queue provider.
    """
...
def mock_job_data():
    ...
    Returns:
        dict: A dictionary containing mock job data with timestamps.
    """
    now = datetime.now()
    return {
        "msg_id": 1,
...
class TestSupabaseQueueProvider:
    ...
    def test_send_message_with_delay(
        self, provider: SupabaseQueueProvider, mock_supabase
    ):
        ...
        mock_supabase.client.schema().rpc.assert_called_with(
            "send",
            {"queue_name": "test_queue", "message": data, "sleep_seconds": delay},
        )
        assert msg_id == 123
    ...
    def test_send_batch_with_delay(
        ...
        mock_supabase.client.schema().rpc.assert_called_with(
            "send_batch",
            {"queue_name": "test_queue", "messages": messages, "sleep_seconds": delay},
        )
        assert msg_ids == [1, 2]

    def test_read_messages(
        self, provider: SupabaseQueueProvider, mock_supabase, mock_job_data
    ):
    ...
    def test_archive_message_by_id(
        self, provider: SupabaseQueueProvider, mock_supabase
    ):
    ...
    def test_general_error_handling(
        ...
        # Setup
        mock_supabase.client.schema().rpc().execute.side_effect = Exception(
            "Some other error"
        )
